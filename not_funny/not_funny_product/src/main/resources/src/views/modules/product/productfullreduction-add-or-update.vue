    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/product/productfullreduction/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.productId = data.productFullReduction.productId
                this.dataForm.fullPrice = data.productFullReduction.fullPrice
                this.dataForm.reducePrice = data.productFullReduction.reducePrice
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/product/productfullreduction/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
